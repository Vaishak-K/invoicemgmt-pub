...
function ViewTable({ db }: any) {
  ...
  const handleDownloadPDF = (ac: any) => {
    ...
    pdf.text(
      `Invoice Date: ${new Date(db?.createdAt).toLocaleDateString()}`,
      120,
      80
    );
    ...
    pdf.text(
      `Order Date: ${new Date(db?.createdAt).toLocaleDateString()}`,
      10,
      105
    );
    pdf.setFontSize(12);
    ...
    if (ac === "pdf") {
      pdf.save(`Invoice_No_${db?.id}-${crypto.randomUUID()}.pdf`);
    } else if (ac === "view") {
  ...
  return (
    <div className="w-full md:w-4/5 mx-auto p-6 md:p-8 bg-white rounded-lg shadow-lg relative">
      <div className="flex justify-between ">
        <h1
          className={` text-white text-lg font-semibold rounded-2xl max-w-36 text-center p-2 ${
            db?.credit <= 0
              ? " bg-green-500"
              : db?.credit > 0 && db?.credit < db?.total
              ? "bg-yellow-500"
              : "bg-red-500"
          }`}
        >
      ...
      <h1 className="mb-4">Created At: {db?.createdAt}</h1>
