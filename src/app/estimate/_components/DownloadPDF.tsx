...
function ViewTable({ db }: any) {
  ...
  let amountInWords = toWords.convert(db?.total);
  ...
  const handleDownloadPDF = (ac: any) => {
    ...
    pdf.text(`Invoice Estimate`, 130, 15);
    ...
    pdf.text(`Date: ${new Date(db?.createdAt).toDateString()}`, 120, 80);

    // Add Invoice ID and Date
    pdf.setFontSize(12);
    pdf.text(`Estimate ID: ${db?.id}`, 10, 100);
    pdf.text(`Date: ${new Date(db?.createdAt).toDateString()}`, 10, 105);
    ...
    sectableRows.push([`Amount in Words: Rupees ${amountInWords} Only`]);
    ...
    if (ac === "pdf") {
      pdf.save(`Estimate - ${db?.id}-${crypto.randomUUID()}.pdf`);
    } else if (ac === "view") {
  ...
  return (
    <div className="w-full md:w-4/5 mx-auto p-6 md:p-8 bg-white rounded-lg shadow-lg relative">
      ...
      <h1 className="font-semibold text-lg">Estimate ID: {db?.id}</h1>
      <h1 className="mb-4">
